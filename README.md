...
Production deployment uses a restricted root SSH administrator against the fixed `/opt/atlas-v2` release directory and `/var/backups/atlas-v2` backup root. Bootstrap does not create an application operator, grant Docker-group membership, or install a delegated `sudo` policy. Restrict the root deployment key at the SSH and infrastructure layers and use it only for the reviewed deployment ceremony.

Every deploy creates an immutable archive of the exact reviewed commit, copies its exact environment file, and stages those inputs with the reviewed coordinator, guardian unit, backup tool, and restore-test tool in a unique root-private directory. One root-owned install/acquisition lock covers remote hash verification, atomic replacement of all four installed operations assets, loaded-unit verification, and durable ownership acquisition. Active bytes are never replaced underneath an active guardian or durable state record. A boot-reconciled lease then fences backup, restore proof, atomic whole-directory release promotion, build/config validation, role rotation, migration, exact permission verification, and writer start. Guarded actions run in recorded process groups; ownership loss terminates the entire group and exact-token containers/database sessions. Backup also fences any exact Atlas migrator before `pg_dump`. See the operations runbook for the complete ceremony and equipped-host gates.
