...
Every deploy creates an immutable archive of the exact reviewed commit, copies its exact environment file, and stages those inputs with the reviewed coordinator, guardian unit, backup tool, restore-test tool, role initializer, and Caddy configuration in a unique root-private directory. One root-owned install/acquisition lock covers remote hash verification, atomic replacement of all six installed operations assets, loaded-unit verification, safe retirement of authenticated recovered state, and durable ownership acquisition. Active bytes are never replaced underneath unresolved durable ownership. A boot-reconciled lease then fences backup, restore proof, atomic whole-directory release promotion with retention of the exact prior tree, build/config validation, role rotation, migration, exact permission verification, and writer start. Production guarded actions run in token-named transient systemd cgroups; ownership loss terminates all descendants plus exact-token containers and database sessions before action identity can clear. Backup also fences any exact Atlas migrator before `pg_dump`. See the operations runbook for the complete ceremony and equipped-host gates.
