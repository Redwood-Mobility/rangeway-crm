...
Every deploy creates an immutable archive of the exact reviewed commit, copies its exact environment file, and stages those inputs with the reviewed coordinator, guardian unit, backup tool, restore-test tool, role initializer, and Caddy configuration in a unique root-private directory. Pre-acquisition failure removes only that exact token stage; if the begin response is lost, the client inspects the root-owned durable state directly under both locks and never executes untrusted installed coordinator bytes. Promotion removes the staged environment as soon as the installed candidate copy is hash-validated; successful completion or recovered-state retirement removes the remaining exact stage. One root-owned install/acquisition lock covers remote hash verification, atomic replacement of all six installed operations assets, loaded-unit verification, safe retirement of authenticated recovered state, and durable ownership acquisition. Active bytes are never replaced underneath unresolved durable ownership. Acquisition snapshots running web and worker containers by exact Docker project/service labels, rejects an unexpected migrator, persists an `activating` claim before guardian startup, and promotes it to `prepared` only after the same-token guardian acknowledgement. A boot-reconciled lease then fences backup, restore proof, atomic whole-directory release promotion with retention of the exact prior tree, build/config validation, role rotation, migration, exact permission verification, writer start, and target/worker/public release readiness. Production guarded actions run in token-named transient systemd cgroups; ownership loss terminates all descendants plus exact-token containers and exact-token database sessions before action identity can clear. Database-session cancellation resolves only the exact labeled running database container and treats an absent database as proof that no matching sessions exist. Completion keeps durable state active until the guardian is disabled and proven inactive, then archives it under the lock. The installed backup helper validates `/opt/atlas-v2` only as the trusted provenance/root-separation identity; backup operations themselves use the exact `atlas-v2` project labels, the exact execution-ready database container and mount, distinct named database/artifact volumes, and direct container operations without Compose or candidate paths. A standalone invocation owns the same root-only host exclusion as the deployment guardian from discovery through restart and cleanup, and refuses active durable ownership, an active guardian, or paused exact-label services before mutation. A coordinator-managed invocation cannot bypass that exclusion by environment claim: under the root-owned state lock it must match the exact token, prepared backup action, action PID/unit, guardian acknowledgement, and live lease recorded in `active.state`. Unreleased zero-state provenance is queried only after all exact web, worker, and migrator writers are fenced, in the same quiesced interval as `pg_dump`. This remains usable when a failed first deployment has restored an empty `/opt/atlas-v2` before candidate promotion. Such a retry uses the distinct `unreleased-v2-foundation` provenance only after proving the complete known Atlas relation set absent and validating the immutable migration-set hash. See the operations runbook for the complete ceremony and equipped-host gates.
