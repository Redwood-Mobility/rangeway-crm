...
6. If an Atlas V2 database already exists, require the pre-deploy backup emitted by the deployment script and pass its restore test before any operator-reviewed data restore.
...
The script validates the clean source tree, runs the local gates, resolves remote paths before mutation, backs up an existing V2 database, synchronizes source without data or secrets, builds, migrates, starts the services, verifies target-bound and public HTTPS health, and only then records `.atlas-release`.
