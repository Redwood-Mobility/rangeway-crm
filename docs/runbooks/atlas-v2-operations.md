...
Each release creates an immutable archive of the exact clean `HEAD`, copies the exact production environment file, hashes both, and transfers them with the reviewed coordinator, guardian unit, backup tool, and restore-test tool into a unique root-private staging directory. The remote install holds `/run/lock/atlas-v2-deployment-install.lock` continuously across hash verification, atomic replacement of all four installed assets, systemd reload/loaded-unit verification, and coordinator `begin`. If installed bytes differ, replacement is allowed only when the guardian is inactive and no durable `active.state` exists. An active deployment must be reconciled or explicitly resolved; never overwrite any installed operations asset in place.
...
- **During or after atomic promotion but before migration:** the same durable, boot-reconciled guardian terminates the recorded action group and restores exactly the captured prior-active writers when the owner fails or the lease expires. Inspect the retained prior release and failure evidence before retrying.
